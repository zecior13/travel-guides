...
const SPEECH_PRONUNCIATIONS = [
  ...
  ,["Blue Note", "Blu Nołt"]
  ,["Brooklyn Tabernacle", "Bruklin Tabernakl"]
  ...
  ,["The Shed", "de Szed"]
  ,["Doug Aitken", "Dag Ejtken"]
  ,["Lightscape", "Lajtskejp"]
  ,["Vessel", "Wesel"]
...
function dayReadingText(day) {
  ...
  const villageExtra = guide?.kind === "village" ? [
    `${guide.gospel.title}. ${guide.gospel.intro}. ${guide.gospel.schedule.join(". ")}`,
  ...
  const momaExtra = guide?.kind === "moma" ? [
    ...
    `${guide.midtown.title}. ${guide.midtown.text}. ${guide.midtown.practical.join(". ")}`,
  ...
  const westExtra = guide?.kind === "westside" ? [...guide.photos.onboard.map(item=>`${item.title}. ${item.text}`),...guide.downtown.route.map(item=>`${item.title}. ${item.text}`),...guide.museum.stops.map(item=>`${item.title}. ${item.text}`),...guide.route.map(item=>`${item.title}. ${item.text}`),...guide.evening.map(item=>`${item.title}. ${item.text}`),...guide.stories.map(item=>`${item.title}. ${item.text}`)].join(" ") : "";
...
function renderVillageGuide(day, guide) {
  ...
  return `
    <nav class="day-module-nav" aria-label="Sekcje dnia">
      ...
      <button class="day-module day-module-feature" type="button" data-day-panel="gospel"><b>♫</b><span>Gospel</span><small>Brooklyn Tabernacle</small></button>
      <button class="day-module" type="button" data-day-panel="guggenheim"><b>◌</b><span>Guggenheim</span><small>2,5 godziny ze sztuką</small></button>
    ...
    <section class="day-panel" data-panel="bluenote" hidden>
      ...
      <div class="countdown-card"><span>Do otwarcia drzwi</span><strong id="blueNoteCountdown">Obliczam…</strong><small>O 17:10 wychodzimy z Caffè Reggio i kierujemy się do klubu.</small></div>
...
function renderMomaGuide(day, guide) {
  return `
    ...
    <section class="day-panel" data-panel="shopping" hidden>
      <h3>${guide.midtown.title}</h3>
      <p class="panel-intro">${guide.midtown.text}</p>
      <article class="event-card"><h4>Krótka wizyta</h4><ul>${guide.midtown.practical.map(item=>`<li>${item}</li>`).join("")}</ul></article>
      <h3 class="subsection-title">Zakupy Matyldy · reguła 2 + 1</h3>
...
function renderWestsideGuide(day, guide) {
  return `<nav class="day-tabs" aria-label="Sekcje dnia"><button class="day-tab active" data-day-panel="overview">Plan</button><button class="day-tab" data-day-panel="ferry">Promy</button><button class="day-tab" data-day-panel="photos">Zdjęcia</button><button class="day-tab" data-day-panel="downtown">Downtown</button><button class="day-tab" data-day-panel="museum">Whitney</button><button class="day-tab" data-day-panel="route">High Line</button><button class="day-tab" data-day-panel="food">Jedzenie</button><button class="day-tab" data-day-panel="evening">The Shed</button><button class="day-tab" data-day-panel="variants">Warianty</button><button class="day-tab" data-day-panel="links">Mapy</button></nav>
  ...
  ${renderFoodPanel(guide)}
  <section class="day-panel" data-panel="evening" hidden><h3>The Shed · podwójne doświadczenie bez przejazdu</h3><p class="panel-intro">Plan główny: Lightscape 17:20–18:25, 35 minut buforu i The Black Mirror Experience 19:00–20:00.</p><div class="guide-grid">${guide.evening.map((x,i)=>guideCard(x,`wevening-${i}`,"Posłuchaj wskazówki")).join("")}</div></section>
  <section class="day-panel" data-panel="variants" hidden><h3>Wybierz wersję dnia</h3><div class="guide-grid">${guide.variants.map(x=>guideCard(x)).join("")}</div></section>
  <section class="day-panel" data-panel="links" hidden><h3>Mapy i oficjalne informacje</h3><div class="link-grid">${guide.links.map(x=>`<a href="${x.url}" target="_blank" rel="noopener">${x.label}<span>↗</span></a>`).join("")}</div><p class="data-note">Rano potwierdzamy piętra Whitney, pogodę na High Line i rozkłady promów; bilety The Shed zapisujemy wcześniej offline.</p></section>`;
}
...
const MODERN_DAY_MODULES = {
  moma:[['overview','01','Plan','godziny i kolejność'],['museum','▣','MoMA','trasa po piętrach'],['works','★','Dzieła','najważniejsze prace'],['animation','●','Animacja','moduł rodzinny'],['shopping','◇','Katedra i Rockefeller','architektura oraz zakupy'],['food','☕','Jedzenie','lunch i piknik'],['movie','▶','Bryant Park','kino pod wieżowcami'],['links','↗','Mapy','trasy i źródła']],
  ...
  queens:[['overview','01','Plan','Midtown, Queens i korty'],['morning','△','Midtown i SUMMIT','Empire, SUMMIT i Grand Central'],['route','◎','Miejsca Queens','LIC, Flushing i Unisphere'],['food','☕','Jedzenie','lunch we Flushing'],['tennis','●','US Open','wejście i stadion'],['basics','?','Tenis 101','zasady dla laików'],['links','↗','Mapy','trasy i powrót']],
  westside:[['overview','01','Plan','port, sztuka i immersja'],['ferry','≈','Promy','dwa rejsy'],['photos','★','Statua','ikoniczne zdjęcia'],['downtown','$','Downtown','Wall Street i memoriał'],['museum','▣','Whitney','sztuka amerykańska'],['route','━','High Line','spacer do The Shed'],['food','☕','Jedzenie','lunch i późna kolacja'],['evening','◈','The Shed','Lightscape i Black Mirror'],['links','↗','Mapy','trasy, bilety i źródła']],
...
const TRIP_REGIONS = [
  ...
  { id: "west", name: "Chelsea i West Side", color: "#4f91b6", text: "Dawny przemysł, High Line, Whitney, The Shed i nowa architektura skierowana ku Hudsonowi.", days: ["2026-08-27"] },
...
const PLACE_EXTRAS = {
  ...
  westside:{region:"west",status:"w planie · pełny wieczór",why:"Whitney, High Line i The Shed pokazują przemianę przemysłowego brzegu w dzielnicę sztuki, spaceru i nowych mediów.",curiosity:"High Line była towarową linią kolejową wprowadzoną bezpośrednio do fabryk i magazynów, a dziś kończy się obok jednej z najmłodszych instytucji sztuki Nowego Jorku.",photoTip:"Na tarasach Whitney połączcie sztukę z panoramą; na High Line szukajcie osi ulic kończących się Hudsonem. W The Shed sprawdźcie zasady fotografowania każdej instalacji.",nearby:["the-shed","summit","downtown"]},
  "the-shed":{region:"west",status:"w planie · 27.08 · 17:20 i 19:00",why:"Zero dodatkowego przejazdu pozwala połączyć siedmioekranową instalację Lightscape z grupowym doświadczeniem Black Mirror bez utraty wieczoru na logistykę.",curiosity:"Ruchoma powłoka The Shed może wysunąć się na plac i zamienić przestrzeń przed budynkiem w ogromną salę wydarzeń.",photoTip:"Przed wejściem sfotografujcie ruchomą konstrukcję od strony Vessel; podczas doświadczeń najpierw sprawdźcie zasady personelu.",nearby:["westside","summit"]},
  "mercer-labs":{region:"downtown",status:"Extra · tylko w NYC · plan B",why:"To mocna wewnętrzna rezerwa na deszcz lub osobny wolny wieczór, ale z Hudson Yards wymaga cofnięcia się do Financial District i jest droższa od Lightscape.",curiosity:"Dawny kompleks handlowy przy Cortlandt Street został przekształcony w piętnaście przestrzeni łączących projekcję, dźwięk, światło i instalację.",photoTip:"Najpierw sprawdźcie zasady konkretnej sali; w ciemnych projekcjach zmniejszcie ekspozycję telefonu i nie używajcie lampy.",nearby:["downtown"]},
  ...
  "st-patrick":{region:"midtown",status:"w planie · 24.08 · 14:30",why:"Krótka wizyta naturalnie łączy MoMA z Rockefeller Center i pokazuje kontrast między XIX-wiecznym neogotykiem a komercyjnymi wieżowcami Fifth Avenue.",curiosity:"Katedrę zaprojektował James Renwick Jr.; otwarto ją w 1879 roku, kiedy okolica wyglądała zupełnie inaczej niż dzisiejsze Midtown.",photoTip:"Z zewnątrz przejdźcie pod Rockefeller Center, aby objąć obie iglice. W środku fotografujcie dyskretnie i bez lampy.",nearby:["rockefeller-shopping","moma","grandcentral"]},
  ...
  "caffe-reggio":{region:"village",status:"w planie · 23.08 · 16:40",why:"To naturalny odpoczynek na końcu spaceru po Village, a jednocześnie miejsce z własną historią, nie przypadkowa kawiarnia.",curiosity:"Caffè Reggio jako pierwsza kawiarnia w USA podawała cappuccino; ozdobny ekspres wykonano w 1902 roku.",photoTip:"Najpierw szeroki kadr zielonej fasady, a wewnątrz detal zabytkowego ekspresu bez przeszkadzania obsłudze.",nearby:["village","bluenote"]},
  "brooklyn-tabernacle":{region:"brooklyn",status:"opcja · 23.08 · 09:00 · tylko przy dobrej energii",why:"To autentyczne doświadczenie gospel, ale po dniu przylotu można je bez żalu pominąć, aby zachować energię na Guggenheim, Blue Note i wieczór.",curiosity:"Brooklyn Tabernacle jest wielokulturową wspólnotą znaną z chóru gospel i oficjalnie zaprasza gości na niedzielne nabożeństwa.",photoTip:"Zdjęcie zróbcie wyłącznie na zewnątrz; podczas nabożeństwa telefony pozostają schowane.",nearby:["guggenheim"]},
...
const DAY_SCHEMATICS = {
  ...
  "2026-08-23": { note: "Plan główny zaczyna się w Guggenheimie; gospel na Brooklynie jest porannym dodatkiem wyłącznie przy dobrej energii.", nodes: [["Hotel", "midtown", "⌂", "overview"], ["Brooklyn Tabernacle · opcja", "brooklyn", "♫", "gospel"], ["Guggenheim", "museum", "◌", "guggenheim"], ["Washington Sq.", "village", "●", "route"], ["Blue Note", "village", "♪", "bluenote"], ["M Social", "midtown", "☾", "rooftop"]], legs: ["metro 30–35 min · opcja", "metro 4/5 + spacer · opcja", "metro/taxi 35–45 min", "spacer historyczny", "taxi/metro ok. 25–35 min"] },
  "2026-08-24": { note: "Dzień Midtown: katedra leży naturalnie między MoMA a Rockefeller Center, a hotel jest bazą przed filmem.", nodes: [["Hotel", "midtown", "⌂", "overview"], ["MoMA", "midtown", "▣", "museum"], ["St. Patrick’s", "midtown", "†", "shopping"], ["Rockefeller", "midtown", "◆", "shopping"], ["Hotel", "midtown", "⌂", "overview"], ["Bryant Park", "midtown", "▶", "movie"]], legs: ["metro/pieszo 20–25 min", "pieszo 5–10 min", "po drugiej stronie Fifth Ave", "pieszo ok. 20 min", "pieszo ok. 12 min"] },
  ...
  "2026-08-26": { note: "Dwie części jednego dnia: ikoniczny Midtown rano, a potem nabrzeże, Flushing i sportowe Queens.", nodes: [["Hotel", "midtown", "⌂", "overview"], ["Empire State", "midtown", "1", "morning"], ["SUMMIT + Grand Central", "midtown", "2", "morning"], ["Long Island City", "queens", "3", "route"], ["Flushing", "queens", "5", "food"], ["Unisphere", "queens", "6", "route"], ["US Open", "queens", "7", "tennis"]], legs: ["pieszo", "pieszo Fifth Avenue", "metro 7 ok. 20 min", "metro linii 7", "spacer/metro", "pieszo ok. 15 min"] },
  "2026-08-27": { note: "Port i Downtown rano, potem Whitney oraz liniowy spacer kończący się dwoma wejściami w The Shed.", nodes: [["Pier 79", "west", "≈", "ferry"], ["St. George", "port", "◁", "photos"], ["Downtown", "downtown", "◆", "downtown"], ["Whitney", "west", "▣", "museum"], ["High Line", "west", "━", "route"], ["Lightscape", "west", "◈", "evening"], ["Black Mirror", "west", "●", "evening"]], legs: ["prom", "darmowy prom", "metro/pieszo", "pieszo", "spacer liniowy", "ten sam budynek · 35 min buforu"] },
...
const DAY_VISUALS = {
  ...
  "2026-08-24": { area:"Midtown", icon:"▣", event:"MoMA · katedra · Bryant Park", color:"#bd694e" },
  ...
  "2026-08-27": { area:"Downtown → West Side", icon:"◈", event:"Statua · Whitney · The Shed", color:"#4f91b6" },
...
const VILLAGE_MAP_STOPS = [
  { panel:"overview", title:"Hotel", note:"Plan główny · spokojny poranek i wyjazd do Guggenheimu", left:43, top:43, icon:"H" },
  { panel:"gospel", title:"Brooklyn Tabernacle", note:"09:00 · opcja tylko przy dobrej energii", left:24, top:93, icon:"♫" },
  { panel:"guggenheim", title:"Guggenheim", note:"11:30–14:00 · pełne 2,5 godziny", left:70, top:11, icon:"◌" },
  { panel:"food", title:"Szybki lunch", note:"14:45 · lekki posiłek bez długiej kolejki", left:33, top:69, icon:"1" },
  { panel:"route", key:"village-stop-0", title:"Washington Sq. i MacDougal", note:"15:10 · łuk, NYU, scena uliczna i folk", left:38, top:76, icon:"2" },
  ...
  { panel:"route", key:"village-stop-2", title:"Stonewall", note:"16:15 · najważniejszy historyczny przystanek", left:53, top:82, icon:"4" },
  { panel:"route", key:"village-stop-3", title:"Caffè Reggio", note:"16:40–17:10 · kawa i odpoczynek", left:61, top:76, icon:"5" },
...
const DAY_MAP_CONFIGS = {
  ...
  "2026-08-23": { title:"Hotel → Guggenheim → Village → Blue Note · gospel opcjonalnie", view:[100,400,650,900], points:[
    {panel:"overview",title:"Hotel",note:"Plan główny · spokojny poranek i wyjazd do Guggenheimu",x:430,y:790,icon:"H"},{panel:"gospel",title:"Brooklyn Tabernacle",note:"09:00 · opcja tylko przy dobrej energii",x:610,y:1210,icon:"♫"},{panel:"guggenheim",title:"Guggenheim",note:"11:30–14:00 · pełne 2,5 godziny",x:590,y:480,icon:"◌"},{panel:"food",title:"Szybki lunch",note:"14:45 · lekki posiłek w Village",x:430,y:960,icon:"1"},{panel:"route",key:"village-stop-0",title:"Washington Sq.",note:"Punkt 2 · łuk, NYU i scena uliczna",x:470,y:985,icon:"2"},{panel:"route",key:"village-stop-1",title:"Jefferson i Commerce",note:"Punkt 3 · kręte ulice West Village",x:410,y:1015,icon:"3"},{panel:"route",key:"village-stop-2",title:"Stonewall",note:"Punkt 4 · historyczny przystanek spaceru",x:450,y:1035,icon:"4"},{panel:"route",key:"village-stop-3",title:"Caffè Reggio",note:"Punkt 5 · 16:40–17:10 · cappuccino i zabytkowy ekspres",x:500,y:1015,icon:"5"},{panel:"bluenote",title:"Blue Note",note:"Hiromi · wejście i koncert",x:495,y:1050,icon:"♪"},{panel:"rooftop",title:"M Social",note:"Opcjonalny drink dorosłych z Anią",x:475,y:710,icon:"☾"},{panel:"rooftop",title:"Hotel · powrót",note:"Matylda czeka w hotelu ze znajomymi",x:430,y:790,icon:"H"}
  ],modes:["transit","transit","transit","walk","walk","walk","walk","walk","transit","walk"]},
  "2026-08-24": { title:"Hotel → MoMA → katedra → Rockefeller → Bryant Park", view:[350,650,300,200], points:[
    {panel:"overview",title:"Hotel · start",note:"Start w Midtown",x:430,y:790,icon:"H"},{panel:"museum",title:"MoMA",note:"10:30 · kolekcja i wystawy",x:500,y:715,icon:"1"},{panel:"food",title:"Lunch",note:"13:30 · szybki posiłek w Midtown",x:520,y:735,icon:"2"},{panel:"shopping",title:"Katedra św. Patryka",note:"14:30 · 15–20 minut wewnątrz",x:535,y:735,icon:"3"},{panel:"shopping",title:"Rockefeller",note:"Zakupy Matyldy i architektura kompleksu",x:525,y:750,icon:"4"},{panel:"overview",title:"Hotel · reset",note:"Koc, bluza i krótki odpoczynek",x:440,y:800,icon:"H"},{panel:"movie",title:"Bryant Park",note:"Piknik i King Creole",x:530,y:805,icon:"5"}
  ],modes:["walk","walk","walk","walk","transit","walk"]},
  ...
  "2026-08-26": { title:"Midtown → Long Island City → Flushing", view:[380,600,482,300], points:[
    {panel:"overview",title:"Hotel",note:"Wczesny start w Midtown",x:430,y:790,icon:"H"},{panel:"morning",key:"morning-stop-1",title:"Empire State",note:"Punkt 1 · widok z zewnątrz i Fifth Avenue",x:500,y:835,icon:"1"},{panel:"morning",key:"morning-stop-2",title:"SUMMIT + Grand Central",note:"Punkt 2 · panorama i Main Concourse",x:555,y:790,icon:"2"},{panel:"route",key:"q-stop-0",title:"Gantry Plaza",note:"Punkt 3 · nabrzeże Long Island City",x:665,y:760,icon:"3"},{panel:"route",key:"q-stop-1",title:"Pepsi-Cola Sign",note:"Punkt 4 · spacer nabrzeżem",x:675,y:720,icon:"4"},{panel:"route",key:"q-stop-3",title:"Flushing",note:"Punkt 5 · jedzenie i migracyjny Nowy Jork",x:835,y:690,icon:"5"},{panel:"route",key:"q-stop-4",title:"Unisphere",note:"Punkt 6 · dziedzictwo World’s Fair",x:805,y:790,icon:"6"},{panel:"tennis",title:"US Open",note:"Punkt 7 · kompleks tenisowy",x:785,y:735,icon:"7"},{panel:"links",title:"Hotel · powrót",note:"Powrót metrem po meczu",x:445,y:800,icon:"H"}
  ],modes:["walk","walk","transit","walk","transit","transit","walk","transit"]},
  "2026-08-27": { title:"Port → Statua → Downtown → Whitney → High Line → The Shed", view:[0,600,862,1100], points:[
    {panel:"overview",title:"Hotel",note:"Start i dojazd do promu",x:430,y:790,icon:"H"},{panel:"ferry",title:"Pier 79",note:"NYC Ferry do St. George",x:365,y:760,icon:"1"},{panel:"ferry",title:"St. George · poza planszą",note:"Terminal leży dalej na południowy zachód. Punkt przy krawędzi pokazuje kierunek pierwszego rejsu.",x:45,y:1670,icon:"→",offmap:true},{panel:"photos",title:"Statua Wolności",note:"Punkt 2 znajduje się na Liberty Island, pośrodku portu; oglądacie ją podczas rejsu powrotnego",x:110,y:1480,icon:"2"},{panel:"downtown",title:"Downtown",note:"Wall Street i 9/11 Memorial",x:405,y:1120,icon:"3"},{panel:"museum",title:"Whitney",note:"13:30–15:30 · sztuka amerykańska",x:420,y:950,icon:"4"},{panel:"route",title:"High Line",note:"15:45 · spacer na północ bez zejścia do galerii",x:420,y:900,icon:"5"},{panel:"evening",title:"Lightscape · The Shed",note:"17:20–18:25 · siedem ekranów i dźwięk",x:405,y:835,icon:"6"},{panel:"evening",title:"Black Mirror · The Shed",note:"19:00–20:00 · fizyczna scenografia i grupowe VR",x:412,y:825,icon:"7"}
  ],modes:["transit","ferry","ferry","ferry","transit","walk","walk","walk"]},
...
function tripMapMarkup() {
  const landmarks = [
    ...
    { id:"brooklyn-tabernacle", icon:"♫", label:"Brooklyn Tabernacle", left:57, top:69 },
    ...
    { id:"the-shed", icon:"◈", label:"The Shed", left:36, top:44 },
  ...
  const regionLabels = [
    ...
    { name:"Brooklyn", days:"Dni 2 + 8", left:77.3, top:69.9 }
  ];
...
const PLACE_DAY_APPEARANCES = {
  ...
  "st-patrick":[{dayId:"2026-08-24",panel:"shopping"}],
  dumbo:[{dayId:"2026-08-29",panel:"brooklyn",focusKey:"brooklyn-stop-0"}],
  heliport:[{dayId:"2026-08-29",panel:"helicopter"}],
  "caffe-reggio":[{dayId:"2026-08-23",panel:"route",focusKey:"village-stop-3"}],
...
const MATYLDA_ATLAS = [
  ...
  { icon:"◈", title:"The Shed · podwójne wow", meta:"Dzień 6 · 27.08", text:"Lightscape o 17:20, bezpieczny bufor i grupowe The Black Mirror Experience o 19:00 — bez dodatkowego przejazdu.", dayId:"2026-08-27", panel:"evening", placeId:"the-shed" },
...
function renderMuseumHub() {
  app.innerHTML = `<div class="view-heading"><h2>Muzea</h2><p>Osobisty przewodnik Gosi: orientacja w budynku, szeroki wybór dzieł i własna lista bez ryzyka, że aplikacja wybierze wszystko za Was.</p></div>
    <div class="notice">Katalog został sprawdzony 15.08.2026 i zawiera wyłącznie dzieła potwierdzone przez muzea jako dostępne na ekspozycji. Przy każdej karcie zachowujemy oficjalny link i lokalizację.</div>
    ...
    <div class="simple-card museum-method"><h3>Jak działa katalog?</h3><p>Najpierw usuwamy wszystko, czego nie da się zobaczyć, a uwolnione miejsce przeznaczamy na dodatkowe, wartościowe dzieła tych samych artystów, które są na salach. W przeddzień każdej wizyty warto jeszcze raz otworzyć oficjalne linki — muzea mogą zmienić ekspozycję bez zapowiedzi.</p></div>`;
...
function renderMuseumDetail(id) {
  ...
  if (museum.works.every(work => work.status === "on")) {
    document.getElementById("museumStatus")?.closest("label")?.remove();
  }
...
function renderMetroPreparePanel() {
  const legs = [
    {line:"Q",color:"#fccc0a",title:"23.08 · plan główny: hotel → Guggenheim",text:"Q z Times Sq–42 St do 86 St, a następnie około 15–20 minut pieszo na zachód. Wyjazd około 10:30; alternatywnie taxi."},
    {line:"2/3",color:"#ee352e",title:"23.08 · opcja: hotel → Brooklyn Tabernacle",text:"Tylko przy dobrej energii: 2 lub 3 z Times Sq–42 St do Borough Hall; dalej około 8–10 minut pieszo. Wyjazd 08:10."},
    {line:"4/5",color:"#00933c",title:"23.08 · opcja: Brooklyn Tabernacle → Guggenheim",text:"Po wyjściu najpóźniej 10:25: 4 lub 5 z Borough Hall do 86 St/Lexington Av; dalej około 12–15 minut pieszo do muzeum."},
    {line:"6",color:"#00933c",title:"23.08 · Guggenheim → Village",text:"Po wyjściu o 14:00: 6 local z 86 St/Lexington Av w kierunku Brooklyn Bridge–City Hall → Astor Pl; dalej spacer do Washington Square."},
